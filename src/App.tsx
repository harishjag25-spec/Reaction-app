  ...
  if (isInitializing) {
    return (
      <div className="h-screen flex items-center justify-center bg-[#0a0a0a]">
        <div className="text-[10px] uppercase tracking-[0.4em] text-stone-600 animate-pulse">Establishing Session...</div>
      </div>
    );
  }
...
function ReactionGame({ user, token }: { user: UserData | null, token: string | null }) {
  ...
  const [standing, setStanding] = useState<{ rank: number, percentile: number, saved: boolean, message?: string } | null>(null);
  ...
  const saveScoreAndFetchRank = useCallback(async (time: number) => {
    try {
      ...
      console.error('Failed to process score and fetch rank:', e);
    }
  }, [token]);
  ...
  const handleInteraction = useCallback(() => {
    ...
    if (gameState === 'WAITING') {
      ...
      setGameState('RESULT');
      saveScoreAndFetchRank(reactionTime);
    } else if (gameState === 'START' || gameState === 'RESULT' || gameState === 'TOO_SOON') {
      startGame();
    }
  }, [gameState, startGame, saveScoreAndFetchRank]);
          ...
          <div className="flex flex-col items-center gap-6 w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
            {standing && (
              <div className="flex flex-col items-center gap-3">
                ...
                {!standing.saved && standing.message && (
                  <div className="text-[10px] text-emerald-400/80 uppercase tracking-[0.2em] font-medium text-center max-w-[200px] leading-relaxed">
                    {standing.message}
                  </div>
                )}
              </div>
    ...
    >
      <div className="flex items-end justify-between w-full mb-12 border-b border-white/5 pb-8 pt-4">
        <div className="flex flex-col">
          ...
          <p className="text-[10px] uppercase tracking-[0.3em] text-stone-500 mt-2">Verified Response Times (Top 250)</p>
        </div>
      </div>
