...
app.get('/api/me', authenticate, (req: any, res) => {
  const user = db.prepare('SELECT username, email FROM users WHERE id = ?').get(req.user.userId) as any;
...
app.post('/api/scores', async (req: any, res) => {
  ...
  if (score < 100) {
    return res.status(400).json({ error: 'Score too unrealistic' });
  }
  ...
  try {
    ...
    if (!isGuest) {
      ...
      // 1.1 Cleanup: maintain only top 250 records overall
      db.prepare(`
        DELETE FROM scores 
        WHERE id NOT IN (
    ...
    // 4. Fetch Top 250
    const top250 = db.prepare('SELECT username, reaction_time as time, timestamp FROM scores ORDER BY reaction_time ASC LIMIT 250').all();
    ...
    res.json({ 
      ...
      rank, 
      percentile, 
      leaderboard: rank <= 250 ? top250 : undefined,
      totalPlayers: total,
      message
    });
  } catch (err) {
...
app.get('/api/leaderboard', (req, res) => {
  try {
    const scores = db.prepare('SELECT username, reaction_time as time, timestamp FROM scores ORDER BY reaction_time ASC LIMIT 250').all();
    res.json(scores);
  } catch (err) {
